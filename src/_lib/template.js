...
function Talk({talk, parsedReq}) {
  const {images} = parsedReq
  const {thumb_nail, instructor: {full_name, avatar_256_url} } = talk
  return (
    <React.Fragment>
      <Global styles={reset} />
      <div
        ...
        >
          ...
          >
            ...
            >
              <img src={thumb_nail} width="460" css={{objectFit: 'cover', height: '100%'}} />
            </div>
            <div
              css={{
                position: 'absolute',
          ...
          >
            ...
            <h1
              css={{
                ...
                lineHeight: 1.2,
                fontSize:
                  talk.title.length > 30
                    ? talk.title.length > 45
                      ? talk.title.length > 55
                        ? 48
                        : 52
                      : 56
                    : 60,
  ...
  switch (parsedReq.resourceType) {
    ...
    case 'talk':
      const talk = await axios
          .get(`https://egghead.io/api/v1/talks/${parsedReq.text}`)
          .then(({ data }) => data)
        markup = renderToStaticMarkup(
          <Talk talk={talk} parsedReq={parsedReq} />,
        )
      break
    default:
      markup = renderToStaticMarkup(
